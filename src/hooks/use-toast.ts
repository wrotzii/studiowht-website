...
export const reducer = (state: any, action: any) => {
  switch (action.type) {
    ...
    case "DISMISS_TOAST": {
      ...
      if (toastId) { addToRemoveQueue(toastId) } else { state.toasts.forEach((toast: any) => { addToRemoveQueue(toast.id) }) }
...
function toast({ ...props }: any) {
  const id = genId()
  const update = (props: any) => dispatch({ type: "UPDATE_TOAST", toast: { ...props, id } })
